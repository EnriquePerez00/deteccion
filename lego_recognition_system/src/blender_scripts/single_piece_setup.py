...
def setup_world_hdri(assets_dir):
    """Sets a pure black world background."""
    print(f"🌍 Applying Pure Black Background")
...
def setup_ground_texture(ground_obj, assets_dir):
    """Sets a perfectly black, non-reflective ground."""
    print(f"🖼️ Applying Solid Black Ground Texture")
...
def _apply_lego_material(obj, color_id=None):
    """Override all materials on obj (and children) with neutral Light Gray plastic."""
    # Force neutral Light Gray (clay render)
    rgba = (0.5, 0.5, 0.5, 1.0)
    print("  🎨 Using Color-Neutral Material (Light Gray)")
...
def get_stable_orientations(obj):
    """
    Analyzes piece geometry using a full 3D physics drop test 
    to find true stable resting positions on a flat surface.
    Returns a list of Euler rotations.
    """
    import math
    import bpy
    ...
    # Ensure obj is configured for physics
    bpy.context.view_layer.objects.active = obj
    ...
    # Get max size to know drop height
    corners = get_hierarchy_corners(obj)
    if corners:
        max_dist = max([(c - obj.location).length for c in corners])
    else:
        max_dist = max(obj.dimensions) / 2.0
        
    drop_z = max_dist + 0.03  # Drop from slightly above max radius

    stable_matrices = []
    
    # 45-degree increments in X and Y, 90-degree in Z = 256 physical tests
    steps_xy = 8
    steps_z = 4
    
    print(f"    [Physics Analysis] Injecting {steps_xy * steps_xy * steps_z} structural drops over 60 frames each...")

    for x_i in range(steps_xy):
        ax = x_i * (math.pi * 2 / steps_xy)
        for y_i in range(steps_xy):
            ay = y_i * (math.pi * 2 / steps_xy)
            for z_i in range(steps_z):
                az = z_i * (math.pi * 2 / steps_z)
                
                # Reset simulation to allow modifying active rigid bodies
                scene.frame_set(1)
                bpy.ops.ptcache.free_bake_all()
                
                # Drop coordinates
                obj.location = (0, 0, drop_z)
                ...
                # Read downward gravity orientation mapping
                final_matrix = obj.matrix_world.to_3x3()
                inv_matrix = final_matrix.inverted()
                down_in_local = inv_matrix @ Vector((0, 0, -1))
                
                is_new = True
                for existing_down, existing_euler in stable_matrices:
                    angle = down_in_local.angle(existing_down)
                    if math.degrees(angle) < 10.0:  # Group similar rests (< 10 deg)
                        is_new = False
                        break
                        
                if is_new:
                    # Capture the stable rotational state without the random yaw
                    clean_euler = Euler((obj.rotation_euler.x, obj.rotation_euler.y, 0.0), 'XYZ')
                    stable_matrices.append((down_in_local, clean_euler))
    ...
    distinct_faces = [euler for (down, euler) in stable_matrices]
...
def main():
    ...
    if render_mode == 'ref_pieza':
        ...
        if target_face_idx >= 0 and len(orientations) == 0:
            print(f"⚠️ Computed stable faces dynamically since they weren't in config.")
            orientations = get_stable_orientations(template_obj)
        ...
        # Hide the ground completely. We don't want any ground shadows
        ground_obj = bpy.data.objects.get("Plane")
        if ground_obj:
            ground_obj.hide_render = True
            
        scene.render.film_transparent = False
        scene.world.node_tree.nodes['Background'].inputs['Color'].default_value = (0.0, 0.0, 0.0, 1.0)
        ...
        for i_iter in range(num_iters):
            if target_face_idx < 0:
                ...
                # Center before placing on ground
                template_obj.location.x = 0
                template_obj.location.y = 0
                place_piece_on_ground(template_obj)

            # Fix num rotations to 24 (15 degree steps)
            rots_per_face = 24
            for r in range(rots_per_face):
                if target_face_idx >= 0:
                    rot_vals = orientations[target_face_idx]
                    template_obj.rotation_euler = (rot_vals[0], rot_vals[1], rot_vals[2])
                    template_obj.rotation_euler[2] += (r * (15.0 * math.pi / 180.0))
                else:
                    template_obj.rotation_euler[2] = (r * (15.0 * math.pi / 180.0))
                
                # Always force exactly center
                template_obj.location.x = 0
                template_obj.location.y = 0
                place_piece_on_ground(template_obj)
                ...
                focal_mm = 26.0
                sensor_half_mm = 18.0  # 36mm full-frame 
                
                # OPTICAL PARITY WITH INFERENCE:
                # The real GrabCut pipeline trims the 24MP image down so the YOLO box
                # occupies exactly 80% of the cropped frame. By mimicking that framing digitally 
                # inside Blender using sensor crop (zoom), we can natively render at 512x512 
                # (super fast) instead of 1024x1024, yet output an image identical in 
                # structure to the 24MP cropped real-world images.
                fill_ratio = 0.40  # 40% radius = 80% diameter occupancy
                
                # Cap the zoom to avoid extreme micro-noise rendering on tiny elements
                effective_radius = max(radius, 0.005)
                
                # Math: Calculate optical distance required to make `effective_radius` 
                # match `fill_ratio` of the current lens/sensor combo.
                target_dist = (effective_radius * focal_mm) / (fill_ratio * sensor_half_mm)
                
                # CRITICAL ALIGNMENT: Camera physically MUST stay fixed at 0.70m height 
                # to preserve perspective/lens distortions exactly as in reality.
                cam_z = 0.70
                real_dist = max(0.01, cam_z - piece_top_z)
                
                # Achieve equivalent framing by digitally cropping the sensor (Zoom)
